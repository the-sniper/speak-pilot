...
This test needs seeded placements. Run one generation against the mock provider before the suite. If the table is empty the test must **fail loudly** with `throw new Error("No placements seeded. Run a generation against the mock provider first.")` in `beforeAll` — never `it.skip`, which reports green while asserting nothing.
